...
#if 0
/**
 * display_variable - XXX
 */
static void display_variable(const char *name, struct HashElem *he, struct Buffer *tmp,
                      struct Buffer *result)
{
  if (!name || !he || !tmp || !result)
    return;
  ...
  cs_he_string_get(Config, he, tmp);
  pretty_var(result, tmp->data);
}
#endif
                     ...
                     struct Buffer *err)
{
  ...
  while (MoreArgs(s))
  {
    ...
    {
      mutt_buffer_printf(err, "ERR22 cannot use 'inv', 'no', '&' or '?' with the '%s' command", set_commands[data]);
      return -1;
    }
    ...
    if (*s->dptr == '?')
    {
      if (prefix)
      {
        mutt_buffer_printf(err, "ERR02 cannot use a prefix when querying a variable");
        return -1;
      }

      if (reset || unset || inv)
      {
        mutt_buffer_printf(err, "ERR03 cannot query a variable with the '%s' command", set_commands[data]);
        return -1;
      }
    ...
    else if (*s->dptr == '=')
    {
      if (prefix)
      {
        mutt_buffer_printf(err, "ERR04 cannot use prefix when setting a variable");
        return -1;
      }

      if (reset || unset || inv)
      {
        mutt_buffer_printf(err, "ERR05 cannot set a variable with the '%s' command", set_commands[data]);
        return -1;
      }
    ...
    if (!bq && (inv || unset))
    {
      if (data == MUTT_SET_SET)
        mutt_buffer_printf(err, "ERR06 prefixes 'no' and 'inv' may only be " "used with bool/quad variables");
      else
        mutt_buffer_printf(err, "ERR07 command '%s' can only be used with bool/quad variables", set_commands[data]);
      return -1;
    }
...
int mutt_query_variables(struct ListHead *queries)
{
  ...
  STAILQ_FOREACH(np, queries, entries)
  {
    ...
    //QWQ should be passed off to dump
    printf("set %s = %s\n", np->data, value->data);
  }
                      ...
                      intptr_t value, struct Buffer *err)
{
  ...
  {
    mutt_buffer_printf(err, _("Option %s may not be set or reset from the pager"), cdef->name);
    return CSR_ERR_INVALID;
  }
                      ...
                      intptr_t value, struct Buffer *err)
{
  ...
  mutt_buffer_printf(err, _("Option %s may not be set when in attach-message mode"), cdef->name);
  return CSR_ERR_INVALID;
}
